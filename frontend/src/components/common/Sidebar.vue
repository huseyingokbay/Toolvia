...
<template>
  ...
  >
    <!-- Logo -->
    <div class="flex items-center h-16 px-4 border-b border-gray-200 dark:border-dark-800">
      <router-link to="/" class="flex items-center gap-3" @click="sidebarStore.closeMobile()">
    ...
    <nav class="flex-1 overflow-y-auto scrollbar-thin py-4 px-2">
      <div v-for="category in categorizedTools" :key="category.id" class="mb-4">
        ...
        <div class="space-y-0.5">
          <router-link
            ...
            :key="tool.id"
            :to="tool.path"
            :class="['sidebar-link', isCurrentPath(tool.path) && 'active']"
            @click="sidebarStore.closeMobile()"
          >
            <span class="w-5 h-5 flex items-center justify-center">
              <!-- Encoding Icons -->
              <svg v-if="tool.icon === 'CodeBracketIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 20l4-16m4 4l4 4-4 4M6 16l-4-4 4-4" />
              </svg>
              <svg v-else-if="tool.icon === 'HashtagIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 20l4-16m2 16l4-16M6 9h14M4 15h14" />
              </svg>
              <svg v-else-if="tool.icon === 'LinkIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
              </svg>
              <svg v-else-if="tool.icon === 'CodeBracketSquareIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <!-- Hash Icon -->
              <svg v-else-if="tool.icon === 'FingerPrintIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 008 11a4 4 0 118 0c0 1.017-.07 2.019-.203 3m-2.118 6.844A21.88 21.88 0 0015.171 17m3.839 1.132c.645-2.266.99-4.659.99-7.132A8 8 0 008 4.07M3 15.364c.64-1.319 1-2.8 1-4.364 0-1.457.39-2.823 1.07-4" />
              </svg>
              <!-- Converter Icons -->
              <svg v-else-if="tool.icon === 'ArrowsRightLeftIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="tool.icon === 'LanguageIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="tool.icon === 'CalculatorIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="tool.icon === 'SwatchIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <!-- Format Icons -->
              <svg v-else-if="tool.icon === 'DocumentTextIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
              </svg>
              <svg v-else-if="tool.icon === 'CircleStackIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.25 6.375c0 2.278-3.694 4.125-8.25 4.125S3.75 8.653 3.75 6.375m16.5 0c0-2.278-3.694-4.125-8.25-4.125S3.75 4.097 3.75 6.375m16.5 0v11.25c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125V6.375m16.5 0v3.75m-16.5-3.75v3.75m16.5 0v3.75C20.25 16.153 16.556 18 12 18s-8.25-1.847-8.25-4.125v-3.75m16.5 0c0 2.278-3.694 4.125-8.25 4.125s-8.25-1.847-8.25-4.125" />
              </svg>
              <!-- Generator Icons -->
              <svg v-else-if="tool.icon === 'KeyIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.75 5.25a3 3 0 013 3m3 0a6 6 0 01-7.029 5.912c-.563-.097-1.159.026-1.563.43L10.5 17.25H8.25v2.25H6v2.25H2.25v-2.818c0-.597.237-1.17.659-1.591l6.499-6.499c.404-.404.527-1 .43-1.563A6 6 0 1121.75 8.25z" />
              </svg>
              <svg v-else-if="tool.icon === 'LockClosedIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="tool.icon === 'DocumentIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="tool.icon === 'SparklesIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <!-- Other Icons -->
              <svg v-else-if="tool.icon === 'QrCodeIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <svg v-else-if="tool.icon === 'DocumentDuplicateIcon'" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
