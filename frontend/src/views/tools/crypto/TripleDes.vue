...
// Mode options
const keySizes: KeySize[] = [128, 192]
...
// Get or generate IV (Triple DES uses 64-bit IV = 8 bytes)
...
const encrypt = () => {
  ...
  try {
    ...
    const encrypted = CryptoJS.TripleDES.encrypt(inputData, key, {
      iv: ivData,
      mode: getCipherMode(),
      padding: getPadding()
    })
...
watch([input, passphrase, rawKey, iv, keySize, cipherMode, padding, keyType, hashAlgorithm, saltType, customSalt, iterations, inputEncoding, outputEncoding], () => {
...
<template>
  <ToolLayout title="Triple DES (3DES) Encryption/Decryption" description="Encrypt and decrypt data using Triple DES algorithm">
    <div class="space-y-6">
      <!-- Mode Toggle -->
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div class="space-y-2">
          ...
          <select v-model="keySize" class="tool-input">
            <option v-for="size in keySizes" :key="size" :value="size">{{ size }} Bits ({{ size === 128 ? '2-Key' : '3-Key' }})</option>
          </select>
        </div>
      ...
      <div class="p-4 bg-gray-50 dark:bg-dark-800 rounded-lg">
        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">About Triple DES (3DES)</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400">
          Triple DES applies the DES algorithm three times to each data block, providing stronger encryption than single DES.
          It supports 128-bit (2-key) and 192-bit (3-key) key sizes. While more secure than DES, AES is recommended for
          modern applications due to its better performance and security.
        </p>
      </div>
    </div>
  </ToolLayout>
