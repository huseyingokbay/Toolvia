...
const getIv = (): CryptoJS.lib.WordArray => {
  ...
  if (autoGenerateIv.value && mode.value === 'encrypt') {
    const newIv = generateRandomHex(16)
    iv.value = newIv
    return CryptoJS.enc.Hex.parse(newIv)
  }

  return CryptoJS.enc.Hex.parse(iv.value.padEnd(32, '0').slice(0, 32))
}
...
const decrypt = () => {
  ...
  try {
    const key = getKey()
    const ivData = CryptoJS.enc.Hex.parse(iv.value.padEnd(32, '0').slice(0, 32))

    // Parse ciphertext based on input encoding
...
<template>
  <ToolLayout title="AES Encryption/Decryption" description="Encrypt and decrypt data using AES algorithm">
    <div class="space-y-6">
      <!-- Mode Toggle -->
      ...
      <div v-if="cipherMode !== 'ECB'" class="space-y-2">
        <div class="flex items-center justify-between">
          <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
            IV (Initialization Vector, 32 hex characters)
          </label>
      ...
      <div class="p-4 bg-gray-50 dark:bg-dark-800 rounded-lg">
        <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">About AES</h3>
        <p class="text-sm text-gray-600 dark:text-gray-400">
          AES (Advanced Encryption Standard) is a symmetric encryption algorithm widely used for secure data encryption.
          It supports key sizes of 128, 192, and 256 bits. For secure password-based encryption, use PBKDF2 key derivation
          with a random salt and at least 10,000 iterations.
        </p>
      </div>
    </div>
  </ToolLayout>
